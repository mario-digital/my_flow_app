...
class TestListFlows:
    ...
    def test_list_flows_returns_paginated_response(
        ...
        with mock_auth_success():
            with (
                patch("src.repositories.context_repository.ContextRepository") as MockContextRepo,
                patch("src.repositories.flow_repository.FlowRepository") as MockFlowRepo,
            ):
                mock_context_repo = MockContextRepo.return_value
                mock_flow_repo = MockFlowRepo.return_value

                mock_context_repo.get_by_id = AsyncMock(
                    return_value=ContextInDB(**mock_context_data)
                )
                mock_flow_repo.count_by_context = AsyncMock(return_value=1)
                mock_flow_repo.get_all_by_context = AsyncMock(
                    return_value=[FlowInDB(**mock_flow_data)]
                )
    ...
    def test_list_flows_returns_403_if_context_not_owned(
        ...
        with mock_auth_success():
            with patch("src.repositories.context_repository.ContextRepository") as MockRepo:
    ...
    def test_list_flows_filters_by_include_completed(
        ...
        with mock_auth_success():
            with (
                ...
                mock_flow_repo.count_by_context = AsyncMock(return_value=0)
                mock_flow_repo.get_all_by_context = AsyncMock(return_value=[])
    ...
    def test_create_flow_success(
        ...
        with mock_auth_success():
            with (
                ...
                mock_flow_repo.create = AsyncMock(return_value=FlowInDB(**mock_flow_data))
...
class TestGetFlow:
    ...
    def test_get_flow_success(
        ...
        with mock_auth_success():
            with patch("src.repositories.flow_repository.FlowRepository") as MockRepo:
                mock_repo = MockRepo.return_value
                mock_repo.get_by_id = AsyncMock(return_value=FlowInDB(**mock_flow_data))
