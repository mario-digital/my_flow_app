...
- [ ] **Task 0: Add Flow Card Tailwind mappings to globals.css** (AC: 3)
  ...
  - [ ] Add Flow Card color mappings to `@theme` block after line 91 (Component Colors - Cards):
    ...
    ```
  - [ ] Verify Tailwind utilities now work: `bg-flow-card`, `border-flow-card-border`, `text-flow-card-text`
      ...
      return (
        <div
          className={cn(
            ...
            "bg-flow-card text-flow-card-text",
          ...
          onClick={onClick}
          // 3px left border in context color (inline style - dynamic value)
          style={{
            borderLeftWidth: '3px',
            borderLeftColor: 'var(--flow-card-border-left)'
          }}
        >
          {children}
        </div>
