...
- [ ] **Subtask 2.2:** Verify "MyFlow Development" vault exists (created in Story 1.1)
- [ ] **Subtask 2.3:** Create new item in vault:
  ```bash
  op item create \
    --category=login \
    --title="MyFlow Logto Frontend" \
...
- [ ] **Subtask 2.5:** Update `my_flow_client/.env.template` to document required variables:
  ...
  # Get these values from 1Password vault "MyFlow Development" > "MyFlow Logto Frontend"
...
- [ ] **Subtask 7.2:** Add `NEXT_PUBLIC_API_URL` and `NEXT_PUBLIC_LOGTO_RESOURCE` to `.env.template`:
  ...
  NEXT_PUBLIC_LOGTO_RESOURCE=op://MyFlow Development/MyFlow Logto Frontend/NEXT_PUBLIC_LOGTO_RESOURCE
  ```
...
- [ ] **Subtask 11.1:** Update `my_flow_client/README.md` with Logto setup instructions:
  ...
  2. Store credentials in 1Password vault "MyFlow Development" > "MyFlow Logto Frontend"
...
- 1Password CLI configured for secret management with vault "MyFlow Development"
...
*(Dev agent will document any blockers encountered and how they were resolved)*

### Architecture Deviations

*(Dev agent will document any deviations from planned architecture with justification)*
...
*(QA engineer will document test execution results here)*

### Issues Found

*(QA engineer will list any bugs or issues discovered)*

### Sign-off

*(QA engineer will sign off when story meets all acceptance criteria)*
