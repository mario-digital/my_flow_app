...
- [ ] **Task 0: Review existing chat and context switcher components** (AC: 1, 4)
  - [ ] Open `my_flow_client/src/components/chat/chat-interface.tsx` (from Story 3.5)
  - [ ] Review `ChatInterfaceProps` interface and current implementation
  - [ ] Open `my_flow_client/src/components/contexts/context-switcher.tsx` (from Story 2.5)
  - [ ] Review `ContextSwitcherProps` interface and `onContextChange` callback
  - [ ] Open `my_flow_client/src/hooks/use-chat-stream.ts` (from Story 3.6)
  - [ ] Understand current message state management
  - [ ] Document integration points in Dev Notes

- [ ] **Task 1: Create Next.js BFF API route for conversation history** (AC: 2)
  - [ ] Create `my_flow_client/src/app/api/conversations/route.ts`
  - [ ] Implement GET handler with BFF proxy pattern:
    ```typescript
    ...
    ```
  - [ ] Add error handling for invalid context_id
  - [ ] Add logging for debugging

- [ ] **Task 2: Create TanStack Query hook for conversation history** (AC: 2)
  - [ ] Create `my_flow_client/src/hooks/use-conversation-history.ts`
  - [ ] Define conversation query key factory:
    ```typescript
    ...
    ```
  - [ ] Implement `useConversationHistory` hook:
    ```typescript
    ...
    ```
  - [ ] Export hook for use in chat interface

- [ ] **Task 3: Make chat interface context-aware** (AC: 1, 3)
  - [ ] Open `my_flow_client/src/components/chat/chat-interface.tsx`
  - [ ] Update component props to accept `contextId`:
    ```typescript
  ...
  - [ ] Merge history with current streaming messages:
    ...
    ```
  - [ ] Pass `conversationId` to `useChatStream` hook:
    ```typescript
    const {
      sendMessage,
      isStreaming,
...
- [ ] **Task 4: Add context switch effect to clear chat** (AC: 1, 4)
  - [ ] Add `useEffect` to handle context changes:
    ```typescript
    ...
    ```
  - [ ] Show loading skeleton while loading history:
    ```typescript
    if (isLoadingHistory) {
      return (
    ...
    ```
  - [ ] Ensure smooth transition with no flickering

- [ ] **Task 5: Update chat-stream hook to support conversation ID** (AC: 3)
  - [ ] Open `my_flow_client/src/hooks/use-chat-stream.ts`
  - [ ] Update hook signature to accept optional `conversationId`:
    ```typescript
    ...
    ```
  - [ ] Include `conversationId` in SSE stream requests:
    ```typescript
    ...
    ```
  - [ ] Backend automatically appends messages to conversation

- [ ] **Task 6: Add conversation history persistence** (AC: 3)
  - [ ] Verify backend endpoint saves messages automatically
  - [ ] Test that conversation history persists across page refreshes
  - [ ] Verify 50-message limit is enforced by backend
  - [ ] Add comment in chat interface explaining persistence:
    ```typescript
    /**
...
- [ ] **Task 7: Create loading skeleton component** (AC: 2, 4)
  - [ ] Create `my_flow_client/src/components/chat/chat-loading-skeleton.tsx`:
    ```typescript
    ...
    ```
  - [ ] Use in chat interface when `isLoadingHistory` is true
  - [ ] Ensure styling matches chat interface design

- [ ] **Task 8: Write integration tests for context switching** (AC: 5)
  - [ ] Create `my_flow_client/__tests__/integration/context-chat-integration.test.tsx`
  - [ ] Test: `test_context_switch_clears_chat()`:
    ```typescript
    ...
    ```
  - [ ] Test: `test_conversation_history_loading()`:
    ```typescript
    ...
    ```
  - [ ] Test: `test_chat_persists_across_refresh()`:
    ```typescript
    ...
    ```
  - [ ] Test: `test_loading_skeleton_displays()`:
    ```typescript
    ...
    ```
  - [ ] Run tests: `bun test __tests__/integration/context-chat-integration.test.tsx`

- [ ] **Task 9: Manual testing with multiple contexts** (AC: 6)
  - [ ] Start development server: `cd my_flow_client && bun dev`
  - [ ] Create at least 2 contexts (Work, Personal) via UI
  - [ ] Send messages in Work context chat
  - [ ] Switch to Personal context
  - [ ] Verify Work chat clears immediately
  - [ ] Verify loading skeleton displays briefly
  - [ ] Verify Personal context starts with empty chat or previous history
  - [ ] Send messages in Personal context
  - [ ] Switch back to Work context
  - [ ] Verify Work conversation history loads correctly
  - [ ] Verify switching feels instant (<200ms)
  - [ ] Refresh page
  - [ ] Verify conversation history persists after refresh
  - [ ] Send 50+ messages to test message limit
  - [ ] Verify oldest messages are truncated
  - [ ] Document manual test results in completion notes

- [ ] **Task 10: Optimize context switch performance** (AC: 4)
  - [ ] Add TanStack Query prefetching for next context:
    ```typescript
    ...
    ```
  - [ ] Measure context switch time with `performance.now()`:
    ```typescript
    ...
    ```
  - [ ] Verify switch completes in <200ms (target)
  - [ ] Add transition animations if time allows

- [ ] **Task 11: Add TypeScript types for conversations** (AC: 2)
  - [ ] Open or create `my_flow_client/src/types/api.ts`
...
- [ ] **Task 12: Run all tests and verify coverage** (AC: 5)
  - [ ] Run integration tests: `bun test __tests__/integration/context-chat-integration.test.tsx`
  - [ ] Run coverage report:
    ```bash
    bun test __tests__/integration/context-chat-integration.test.tsx \
        --coverage
    ```
  - [ ] Verify coverage ≥ 80% for context-chat integration
  - [ ] Fix any failing tests
  - [ ] Run full frontend test suite: `bun test`
...
- [ ] **Task 13: Code quality and compliance** (AC: All)
  ...
  - [ ] Ensure BFF pattern followed (no direct FastAPI calls)
  - [ ] Verify CSS design tokens used (no hardcoded colors/spacing)
