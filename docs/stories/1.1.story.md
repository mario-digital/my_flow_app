...
- [ ] **Task 1: Create Feature Branch** (AC: N/A - Development Workflow)
  - [ ] Create new branch from main: `git checkout -b story-1-1-project-structure`
  - [ ] Verify branch creation: `git branch` should show current branch as `story-1-1-project-structure`
  - [ ] All subsequent work should be done on this branch

- [ ] **Task 2: Verify and Complete Monorepo Structure** (AC: 1)
  - [ ] Verify root directory `my_flow_app/` exists (already created)
  - [ ] Create subdirectories: `my_flow_client/`, `my_flow_api/`, `scripts/`
  - [ ] Verify `.github/workflows/` exists (already present at root)
  - [ ] Verify `docs/` directory exists (already contains PRD and architecture)
  - [ ] Verify and update `.gitignore` in root - ensure it includes patterns from architecture spec (file already exists, add any missing patterns)
  - [ ] Verify Git repository is initialized (already initialized)

- [ ] **Task 3: Initialize Backend with uv** (AC: 4)
  - [ ] Navigate to `my_flow_api/` directory
  - [ ] Run `uv init` to initialize project with Python 3.12+ requirement
  - [ ] Add dependencies via `uv add`: fastapi, pydantic, motor, uvicorn[standard]
  - [ ] Add dev dependencies via `uv add --dev`: pytest, pytest-asyncio, pytest-cov, mypy, ruff
  - [ ] Verify `pyproject.toml` was created with correct dependencies
  - [ ] Create `src/` directory structure: `main.py`, `config.py`, `database.py`, `models/`, `routers/`, `services/`, `repositories/`, `middleware/`, `adapters/`
  - [ ] Create `tests/` directory structure: `unit/`, `integration/`, `conftest.py`
  - [ ] Create `pytest.ini` with test configuration
  - [ ] Verify uv environment: `uv run python --version` should show Python 3.12+

- [ ] **Task 4: Initialize Frontend with Bun + Next.js** (AC: 5)
  - [ ] Run `bun create next-app my_flow_client` with TypeScript and Tailwind CSS
  - [ ] Configure Next.js 15 with App Router in `next.config.js`
  - [ ] Install shadcn/ui CLI: `bunx shadcn@latest init`
  - [ ] Configure `components.json` for shadcn/ui with import alias `@/components`
  - [ ] Update `tsconfig.json` with strict mode and path aliases
  - [ ] Create `src/` directory structure: `app/`, `components/`, `lib/`, `hooks/`, `types/`, `styles/`
  - [ ] Configure Tailwind CSS in `tailwind.config.ts` to consume CSS custom properties
  - [ ] **Configure ESLint:**
    - [ ] Verify `eslint.config.mjs` or `.eslintrc.json` exists (created by Next.js)
    - [ ] Add Next.js recommended config and accessibility plugin (eslint-plugin-jsx-a11y)
    - [ ] Configure import order rules for consistent imports
  - [ ] **Configure Prettier:**
    ...
    - [ ] Create `.prettierignore` with patterns: `.next/`, `node_modules/`, `dist/`, `build/`
    - [ ] Install eslint-config-prettier: `bun add -d eslint-config-prettier` to avoid conflicts
    - [ ] Add format script to `package.json`: `"format": "prettier --write \"src/**/*.{ts,tsx,js,jsx,json,css,md}\"` and `"format:check": "prettier --check \"src/**/*.{ts,tsx,js,jsx,json,css,md}\""`

- [ ] **Task 5: Configure Root Workspace** (AC: 1)
  - [ ] Create root `package.json` with Bun workspaces configuration
  - [ ] Add workspace paths: `["my_flow_client", "packages/*"]`
  - [ ] Add convenience scripts using `uv run` for backend: `dev`, `dev:frontend`, `dev:backend`, `test`, `lint`, `format`, `typecheck`, `build`
  - [ ] Create root `bun.lockb` by running `bun install`

- [ ] **Task 6: Create Documentation** (AC: 3)
  ...
  - [ ] Create `my_flow_client/README.md` with:
  ...
  - [ ] Create `my_flow_api/README.md` with:
...
- [ ] **Task 7: Configure 1Password CLI Integration** (AC: 6, 7)
  - [ ] Install 1Password CLI (`op`) on local machine (provide install instructions in README)
  - [ ] Authenticate: `op signin`
  - [ ] Create 1Password vault: `op vault create "MyFlow Development"`
  - [ ] Create 1Password items with placeholders:
  ...
  - [ ] Create `.env.template` files in root, `my_flow_client/`, and `my_flow_api/` with 1Password secret references
  - [ ] Create `scripts/load-secrets.sh` with `op run` wrapper examples

- [ ] **Task 8: Verify and Update .gitignore** (AC: 2)
  - [ ] Verify root `.gitignore` exists (already present)
  - [ ] Ensure `.gitignore` includes required patterns (add if missing):
...
- [ ] **Task 9: Verify Local Development Setup** (AC: 8)
  - [ ] Start backend: `cd my_flow_api && op run --env-file=../.env.template -- uv run uvicorn src.main:app --reload --host 0.0.0.0 --port 8000`
  - [ ] Verify backend health check at `http://localhost:8000/api/v1/health` returns 200
  - [ ] Start frontend: `cd my_flow_client && op run --env-file=../.env.template -- bun dev`
  - [ ] Verify frontend loads at `http://localhost:3000` with default Next.js page
  - [ ] Verify no secrets are committed to Git: `git status` should show no `.env` files

- [ ] **Task 10: Unit Testing Setup** (AC: Testing requirements)
  - [ ] Backend: Create sample test `tests/unit/test_main.py` that tests FastAPI app creation
  - [ ] Backend: Run `uv run pytest` and verify tests pass
  - [ ] Frontend: Create sample test `src/app/page.test.tsx` that renders default page
  - [ ] Frontend: Run `bun test` and verify tests pass
  - [ ] Verify test coverage meets 80% threshold (though minimal at this stage)
