...
- [ ] **Task 2: Implement Context Pydantic models** (AC: 1)
  ...
  - [ ] Add `@field_serializer('id')` to serialize ObjectId to string (Pydantic v2 - no json_encoders)
  - [ ] Create `ContextResponse` as alias or identical to `ContextInDB` for API responses
  - [ ] Add Config class with `from_attributes = True` (Pydantic v2 - json_encoders deprecated)
...
- [ ] **Task 3: Implement Flow Pydantic models** (AC: 2)
  ...
  - [ ] Add `@field_validator('due_date')` to enforce timezone-aware datetime (UTC required)
