...
- [ ] **Task 2: Extend FlowRepository with bulk delete method** (Prerequisite for cascade delete)
  - [ ] **IMPORTANT: This method was not in Story 2.3 but is required for Story 2.4 cascade delete**
  - [ ] Add `delete_by_context_id()` method to FlowRepository in `my_flow_api/src/repositories/flow_repository.py`
  ...
  - [ ] **Why this is needed:**
    - Avoids N+1 query problem (fetch all flows, delete one-by-one)
    - Single delete_many() query regardless of flow count
    - Performance: 100 flows = 101 queries → 2 queries
  - [ ] Add unit tests for `delete_by_context_id()`:
    - Test successful bulk delete (create 10 flows, delete all)
    - Test ownership verification (returns 0 if user doesn't own context)
    - Test returns correct count
    - Test with 0 flows (returns 0)
  - [ ] Update Story 2.3 implementation if needed (or document as Story 2.4 extension)
...
- [ ] **Task 5: Register routers and configure rate limiting** (AC: 1, 2)
