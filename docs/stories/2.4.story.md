...
- [ ] **Task 1: Create auth middleware dependency** (AC: 3)
  ...
  - [ ] Implement `get_current_user()` dependency that validates Logto JWT
  ...
  - [ ] **SECURITY: Implement TTL-based JWKS caching (1-hour TTL)**
...
- [ ] **Task 2: Create Context API router** (AC: 1, 3, 4)
  ...
  - [ ] Implement `DELETE /api/v1/contexts/{context_id}` - Delete context
    - Use `Depends(get_current_user)` to get `user_id`
    - First delete all flows for context using `flow_repo.delete_by_context(context_id, user_id)`
...
**When deleting context, must delete all flows first:**
```python
...
async def delete_context(
    ...
    # Delete all flows for context
    flows = await flow_repo.get_all_by_context(context_id)
    for flow in flows:
        await flow_repo.delete(flow.id)
