...
2. **TanStack Query hooks created in `my_flow_client/src/hooks/use-contexts.ts`:**
   ...
   - `useDeleteContext()` → Uses `useMutation` with optimistic update and cache invalidation
...
- [ ] **Task 3: Define Query Keys factory** (AC: 4)
  ...
  - [ ] Export `contextKeys` for use in tests and components
...
- [ ] **Task 4: Implement useContexts() query hook** (AC: 2, 3)
  ...
  - [ ] Get current user ID (use placeholder for now, will integrate with Logto in future)
...
- [ ] **Task 6: Implement useCreateContext() mutation hook with optimistic update** (AC: 2, 3)
  ...
  - [ ] Implement `useCreateContext()` hook:
    ...
    export function useCreateContext() {
      ...
      const userId = 'current-user-id'; // TODO: Get from Logto
  ...
  - [ ] **Optional:** Add `mutationKey: ['createContext']` if rapid duplicate mutations are a concern (see Dev Notes: Race Condition Handling)
...
- [ ] **Task 7: Implement useUpdateContext() mutation hook with optimistic update** (AC: 2, 3)
  ...
  - [ ] Implement `useUpdateContext()` hook:
    ...
    export function useUpdateContext() {
      ...
      return useMutation({
        mutationFn: ({ id, data }: { id: string; data: ContextUpdate }) =>
  ...
  - [ ] **Optional:** Add `mutationKey: ['updateContext']` if rapid duplicate mutations are a concern (see Dev Notes: Race Condition Handling)
...
- [ ] **Task 8: Implement useDeleteContext() mutation hook with optimistic update** (AC: 2, 3)
  ...
  - [ ] **Optional:** Add `mutationKey: ['deleteContext']` if rapid duplicate mutations are a concern (see Dev Notes: Race Condition Handling)
...
├── types/
│   └── context.ts               # NEW: Context TypeScript interfaces
└── mocks/
...
The story implements basic race condition prevention using `cancelQueries`:
...
This cancels in-flight queries before optimistic updates, preventing most race conditions.

**Optional Enhancement for Rapid Mutations:**

If users can trigger rapid consecutive mutations (e.g., double-clicking create button), consider adding `mutationKey` to prevent duplicate mutations:

```typescript
export function useCreateContext() {
  return useMutation({
    mutationKey: ['createContext'], // Prevents duplicate mutations in queue
    mutationFn: (data: ContextCreate) => createContext(data),
    onMutate: async (newContext) => {
      await queryClient.cancelQueries({ queryKey: contextKeys.list(userId) });
...
**When to Use Mutation Keys:**
- **Use if**: UI allows rapid repeated actions (double-clicks, keyboard shortcuts)
- **Skip if**: UI has proper loading states and disabled buttons during mutations
- **Default**: Start without `mutationKey`, add only if race conditions occur in testing

**Current Story Approach:**
- Basic implementation uses `cancelQueries` only (sufficient for most cases)
- TanStack Query handles most race conditions automatically
- Developers can add `mutationKey` during implementation if needed

[Source: TanStack Query best practices, coding standards]

---

### User ID Resolution (Placeholder for Now)

**Current Limitation:**

This story uses a **placeholder user ID** because Logto user context integration is not yet complete:
...
Will integrate with Logto user context:
...
**For This Story:**
- Use placeholder `'current-user-id'` in all hooks
- Add TODO comments for future Logto integration
- Ensure `enabled: !!userId` guard is in place
