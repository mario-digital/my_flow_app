...
5. **TypeScript types generated from backend OpenAPI schema (no manual duplication):**
   ...
   - Story code imports `Context`, `ContextCreate`, `ContextUpdate` from generated OpenAPI typings (e.g., via `components['schemas']['ContextResponse']` aliases)
   - Any convenience re-exports simply alias the generated types—no hand-written interfaces allowed
...
- [ ] **Task 1: Create Context TypeScript re-exports (no manual interfaces)** (AC: 5)
  - [ ] Create `my_flow_client/src/types/context.ts`
  - [ ] Import generated schema types: `import type { components } from '@/types/api'`
  - [ ] Export aliases:
    ```typescript
    export type Context = components['schemas']['ContextResponse'];
    export type ContextCreate = components['schemas']['ContextCreate'];
    export type ContextUpdate = components['schemas']['ContextUpdate'];
    ```
    (Adjust keys if schema names differ; confirm via generated file.)
  - [ ] Add top-of-file comment referencing coding standards Section 1 (backend is source of truth)
  - [ ] Ensure file exports only aliases/documentation—no duplicated property lists
...
- [ ] **Task 2: Create Context API client functions** (AC: 1)
  ...
  - [ ] Implement `fetchContextById(id: string)`:
    - Decide whether 404 means “missing context” or “error” for current feature (document decision in Dev Notes or component using the client)
    - **Option A (default):** Re-throw `AppError` and let TanStack Query surface via `onError`
    - **Option B (404 tolerant):** Convert only 404 to `null`, re-throw everything else; document callers must handle `null`
    - Example Option B pattern:
      ```typescript
      ...
      return useMutation({
        ...
        onSuccess: (createdContext) => {
          ...
          queryClient.setQueryData<Context[]>(
            ...
            (old) =>
              old?.map((ctx) =>
                ctx.id.startsWith('temp-') ? createdContext : ctx
              ) ?? []
          );
...
├── types/
│   └── context.ts               # NEW: Context TypeScript aliases (generated OpenAPI re-exports)
└── mocks/
