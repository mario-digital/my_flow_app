...
- Query hooks (`useContexts`, `useContext`) must surface fetch failures via `onError` (using `transformError` + toast) so UI receives consistent feedback
...
- [ ] **Task 4: Implement useContexts() query hook** (AC: 2, 3)
  ...
  - [ ] Import `transformError` from `@/lib/errors`, `toast` from `sonner`, and `CONTEXT_MESSAGES` from `@/lib/messages/contexts`
...
- [ ] **Task 10: Write integration tests for useContexts hook** (AC: 6)
  ...
  - [ ] Test: `useContexts() handles fetch error`
    ...
    - Assert `toast.error` called with message from `transformError` (fallback to `CONTEXT_MESSAGES.fetchListError` when undefined)
...
- [ ] **Task 11: Write integration tests for useContext hook** (AC: 6)
  ...
  - [ ] Test: `useContext(id) handles 404 not found`
    ...
    - Assert `toast.error` called with fallback message (`CONTEXT_MESSAGES.fetchDetailError`)
  - [ ] Test: `useContext(id) handles 401 unauthorized`
    - Override MSW handler to return 401
    - Render hook
    - Wait for error state
    - Assert `toast.error` called with `CONTEXT_MESSAGES.fetchDetailError`
