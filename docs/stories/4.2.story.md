...
- [ ] **Task 0: Review existing flow service implementation** (AC: 1)
  - [ ] Open `my_flow_api/src/services/flow_service.py` (from Epic 2)
  - [ ] Review `FlowService` class structure
  - [ ] Understand `get_flows_by_context()` method
  - [ ] Review flow status computation logic
  - [ ] Check `is_completed` and `due_date` field handling
  - [ ] Document flow urgency determination logic

- [ ] **Task 1: Create transition Pydantic models** (AC: 2)
  - [ ] Create `my_flow_api/src/models/transition.py`
  - [ ] Define `TransitionSuggestions` response model:
    ```python
...
- [ ] **Task 2: Create transition service with suggestion logic** (AC: 1)
  - [ ] Create `my_flow_api/src/services/transition_service.py`
  - [ ] Import required dependencies:
    ...
    ```
  - [ ] Define `TransitionService` class:
    ```python
    ...
    ```
  - [ ] Implement `get_transition_suggestions()` method:
    ```python
...
- [ ] **Task 3: Implement warning generation logic** (AC: 1)
  - [ ] Add `_generate_warnings()` helper method:
    ```python
    def _generate_warnings(
        ...
        return warnings
    ```
  - [ ] Handle singular vs plural flow counts
  - [ ] Highlight high-priority flows in warnings

- [ ] **Task 4: Implement urgent flow identification** (AC: 1)
  - [ ] Add `_identify_urgent_flows()` helper method:
    ```python
    def _identify_urgent_flows(
        ...
        return urgent
    ```
  - [ ] Check for overdue flows (due_date < now)
  - [ ] Check for flows due today (due_date <= end of today)
  - [ ] Check for high-priority flows due within 3 days
  - [ ] Include high-priority flows without due dates
  - [ ] Sort urgent flows by due date (overdue first)
  - [ ] Handle timezone-aware datetime comparisons

- [ ] **Task 5: Implement suggestion generation logic** (AC: 1)
  - [ ] Add `_generate_suggestions()` helper method:
    ```python
    def _generate_suggestions(
        ...
        return suggestions
    ```
  - [ ] Check for overdue flows
  - [ ] Check for flows due today
  - [ ] Generate positive message when no incomplete flows
  - [ ] Handle singular vs plural flow counts

- [ ] **Task 6: Create transitions API router** (AC: 3)
  - [ ] Create `my_flow_api/src/routers/transitions.py`
  - [ ] Import required dependencies:
    ```python
  ...
  - [ ] Create router instance:
    ...
    ```
  - [ ] Define dependency injection for service:
    ```python
  ...
  - [ ] Implement GET endpoint:
    ...
    ```
  - [ ] Use query parameter aliases (`from` and `to`)
  - [ ] Add comprehensive docstring with example
  - [ ] Add error handling

- [ ] **Task 7: Register transitions router in main app** (AC: 3)
  - [ ] Open `my_flow_api/src/main.py`
  - [ ] Import transitions router:
    ```python
  ...
  - [ ] Register router with app:
    ...
    ```
  - [ ] Ensure router is registered after database connection
  - [ ] Verify router appears in OpenAPI docs at `/docs`

- [ ] **Task 8: Write unit tests for transition service** (AC: 4)
  - [ ] Create `my_flow_api/tests/unit/services/test_transition_service.py`
  - [ ] Import test dependencies:
    ...
    from src.models.flow import FlowResponse
    ```
  - [ ] Create mock repositories fixture:
    ```python
    @pytest.fixture
    def mock_flow_repo():
        return AsyncMock()
  ...
  - [ ] Test: `test_no_incomplete_flows()`:
    ...
    ```
  - [ ] Test: `test_incomplete_flows_in_source_context()`:
    ```python
    @pytest.mark.asyncio
    ...
    ```
  - [ ] Test: `test_urgent_flows_due_today()`:
    ```python
    @pytest.mark.asyncio
  ...
  - [ ] Test: `test_overdue_flows()`:
    ...
    ```
  - [ ] Test: `test_high_priority_without_due_date()`:
    ```python
    @pytest.mark.asyncio
    ...
    ```
  - [ ] Run tests: `cd my_flow_api && pytest tests/unit/services/test_transition_service.py -v`

- [ ] **Task 9: Write integration tests for API endpoint** (AC: 3, 4)
  - [ ] Create `my_flow_api/tests/integration/test_transitions_router.py`
  - [ ] Import test dependencies:
    ```python
    import pytest
    ...
    from src.database import get_database
    ```
  - [ ] Test: `test_get_suggestions_endpoint()`:
    ```python
    @pytest.mark.asyncio
  ...
  - [ ] Test: `test_unauthorized_access()`:
    ...
    ```
  - [ ] Test: `test_missing_query_params()`:
    ```python
    @pytest.mark.asyncio
    ...
    ```
  - [ ] Run tests: `cd my_flow_api && pytest tests/integration/test_transitions_router.py -v`
...
- [ ] **Task 11: Manual testing with API client** (AC: 3)
  - [ ] Start backend server: `cd my_flow_api && python -m src.main`
  - [ ] Open API docs: `http://localhost:8000/docs`
  - [ ] Authenticate with test user (JWT token)
  - [ ] Create test contexts via `/api/v1/contexts` (if not exist)
  - [ ] Create test flows with different priorities and due dates:
