...
- [ ] **Task 0: Audit codebase and implement TypeScript enums** (AC: All)
  - [ ] Create `src/types/enums.ts` file for centralized enum definitions
  - [ ] Define required enums matching backend Python enums:
    ```typescript
  ...
  - [ ] Update existing TypeScript interfaces in `src/types/` to use enum types instead of string literals
  - [ ] Ensure all new code in this story uses enums for fixed value sets
